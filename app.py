...
if __name__=="__main__": # used in Python scripts to ensure that certain code is only executed when the script is run directly, not when it is imported as a module.
    st.header("House Price Prediction") # Adds a header to the app UI
    col1,col2 = st.columns([2,1]) # Creates two columns for layout purposes
    bedrooms = col1.slider("NO. of Bedrooms",max_value=10,min_value=1) # Slider for selecting the number of bedrooms
    bathrooms = col1.slider("NO. of Bathrooms",max_value=10,min_value=1) # Slider for selecting the number of bathrooms
    status = col1.selectbox("Select Status:",list(Status.keys())) # Selectbox for selecting the status from a list
    size = col2.number_input("Enter SQFT:",min_value=500,max_value=10000,value=1000,step=500) 
    location = col2.selectbox("Select Location:",list(Location.keys())) # Selectbox for selecting the Location from a list
    facing = col2.selectbox("Select Facing:",list(Facing.keys())) # Selectbox for selecting the Facing from a list
    Types = col1.selectbox("Select Type:",list(Type.keys())) # Selectbox for selecting the Property Type from a list
    result = predict(bedrooms,bathrooms,status,size,location,facing,Types) # Calls the 'predict' function with the selected inputs and assigns the result to 'result'
    submit_button = st.button("Predict") # Button for submitting the User Data
    if submit_button:
        # Formats the result as a larger text with a blue color
        larger_text = f"<h2 style='color: blue;'>The Predicted Price is : {result} Lakhs</h2>"
        st.markdown(larger_text,unsafe_allow_html=True) # Displays the result as markdown
